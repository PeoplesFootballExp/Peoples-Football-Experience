...
func handle_input_event(event: InputEvent) -> void:
	var actions = ["action_a", "action_b", "action_x", "action_y"]
...
func _finalize_command(action_name: String, power: float, is_double: bool) -> void:
	...
	# Aiming snapshot
	cmd.aim_vector = Input.get_vector("ls_left", "ls_right", "ls_up", "ls_down")
...
func _broadcast_movement() -> void:
	var move_vec = Input.get_vector("ls_left", "ls_right", "ls_up", "ls_down")
...
func _update_modifiers() -> void:
	var mask = 0
	if Input.is_action_pressed("mod_lb"): mask |= 1
	if Input.is_action_pressed("mod_rb"): mask |= 2
	if Input.is_action_pressed("mod_lt"): mask |= 4
	if Input.is_action_pressed("mod_rt"): mask |= 8
	_modifier_mask = mask

func _get_action_enum(action_name: String) -> int:
	match action_name:
		"action_a": return GameCommand.ActionType.PASS
		"action_b": return GameCommand.ActionType.SHOOT
		"action_x": return GameCommand.ActionType.KNOCK_ON
		"action_y": return GameCommand.ActionType.THROUGH_BALL
